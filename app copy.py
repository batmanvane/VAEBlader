...
def update_analysis(
    ...
    fig_geo.update_layout(
        title="Geometry", yaxis=dict(scaleanchor="x", scaleratio=1, range=[-0.5, 0.5]),
    ...
    <div style="background-color: #f0f2f6; padding: 15px; border-radius: 10px; border: 1px solid #e5e7eb;">
        <div style="display: flex; justify-content: space-around; text-align: center;">
            <div><div style="color: #6b7280 !important; font-size: 0.9em;">LIFT (CL)</div><div style="color: #111827 !important; font-size: 1.2em; font-weight: bold;">{cl_pt:.4f}</div></div>
            <div><div style="color: #6b7280 !important; font-size: 0.9em;">DRAG (CD)</div><div style="color: #111827 !important; font-size: 1.2em; font-weight: bold;">{cd_pt:.5f}</div></div>
            <div><div style="color: #6b7280 !important; font-size: 0.9em;">MOMENT (CM)</div><div style="color: #111827 !important; font-size: 1.2em; font-weight: bold;">{cm_pt:.4f}</div></div>
            <div><div style="color: #059669 !important; font-size: 0.9em;">L/D RATIO</div><div style="color: #059669 !important; font-size: 1.4em; font-weight: bold;">{ld_pt:.1f}</div></div>
        </div>
    </div>
    """
    ...
    if show_advanced:
        ...
        fig_contour.add_trace(go.Contour(
            z=ld_grid, x=c_rng, y=t_rng, colorscale='Viridis', colorbar=dict(title='L/D', thickness=15),
...
with gr.Blocks(title="AG-VAE Designer", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("Model Diagnostics"):
            ...
            gr.Markdown("These plots are generated during the VAE training process.")

            with gr.Row():
                with gr.Column():
                    gr.Markdown("**Latent Disentanglement**")
                    img_corr = gr.Image(value=f"{PLOT_DIR}/correlation.png", label="Latent-Physics Correlation",
                                        type="filepath")
                    img_align = gr.Image(value=f"{PLOT_DIR}/pivae_alignment_analysis.png", label="Detailed Alignment",
                                         type="filepath")

                with gr.Column():
                    gr.Markdown("**Generative Quality**")
                    img_trav = gr.Image(value=f"{PLOT_DIR}/traversals.png", label="Latent Traversals", type="filepath")
                    img_rec = gr.Image(value=f"{PLOT_DIR}/reconstruction.png", label="Reconstruction Quality",
                                       type="filepath")
